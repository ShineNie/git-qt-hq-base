 ...
 * Static Functions
 * */
QByteArray HQ_Base_Encrypt_AES::Crypt(HQ_Base_Encrypt_AES::Aes level, HQ_Base_Encrypt_AES::Mode mode, const QByteArray &rawText,
                                      const QByteArray &key, const QByteArray &iv, HQ_Base_Encrypt_AES::Padding padding)
{
    return HQ_Base_Encrypt_AES(level, mode, padding).encode(rawText, key, iv);
}
QString HQ_Base_Encrypt_AES::Crypt(HQ_Base_Encrypt_AES::Aes level, HQ_Base_Encrypt_AES::Mode mode,
                                   const QString &sText, const QString &sKey)
{
    QByteArray baText = sText.toLatin1();
    QByteArray baKey = f_MadeKey(sKey).toLatin1();

    QByteArray baEncrypt = Crypt(level, mode, baText, baKey);
    baEncrypt = RemovePadding(baEncrypt);

    QString sEnCrypt = Hq_Base_ModBus::f_ByteArrayToHexStr(baEncrypt);

    return sEnCrypt;
}
QString HQ_Base_Encrypt_AES::f_Encrypt(HQ_Base_Encrypt_AES::Aes eLevel, HQ_Base_Encrypt_AES::Mode eMode,
                                     const QString &sText, const QString &sKey)
{
    return Crypt(eLevel, eMode, sText, sKey);
}
QString HQ_Base_Encrypt_AES::f_Encrypt(const QString &sText, const QString &sKey)
{
    return Crypt(AES_128, ECB, sText, sKey);
}
QByteArray HQ_Base_Encrypt_AES::Decrypt(HQ_Base_Encrypt_AES::Aes level, HQ_Base_Encrypt_AES::Mode mode, const QByteArray &rawText,
                                        const QByteArray &key, const QByteArray &iv, HQ_Base_Encrypt_AES::Padding padding)
{
    return HQ_Base_Encrypt_AES(level, mode, padding).decode(rawText, key, iv);
}
QString HQ_Base_Encrypt_AES::Decrypt(HQ_Base_Encrypt_AES::Aes level, HQ_Base_Encrypt_AES::Mode mode,
                             const QString &sEncrypt, const QString &sKey)
{
    QByteArray baEncrypt = Hq_Base_ModBus::f_HexStrToByteArray(sEncrypt);
    QByteArray baKey = f_MadeKey(sKey).toLatin1();

    QByteArray decodeText = Decrypt(level, mode, baEncrypt, baKey);
    decodeText = RemovePadding(decodeText);

    return QString(decodeText);
}
QString HQ_Base_Encrypt_AES::f_Decrypt(HQ_Base_Encrypt_AES::Aes sLevel, HQ_Base_Encrypt_AES::Mode sMode,
                                       const QString &sEncrypt, const QString &sKey)
{
    return Decrypt(sLevel, sMode, sEncrypt, sKey);
}
QString HQ_Base_Encrypt_AES::f_Decrypt(const QString &sEncrypt, const QString &sKey)
{
    return Decrypt(AES_128, ECB, sEncrypt, sKey);
}
QByteArray HQ_Base_Encrypt_AES::ExpandKey(HQ_Base_Encrypt_AES::Aes level, HQ_Base_Encrypt_AES::Mode mode, const QByteArray &key)
{
     return HQ_Base_Encrypt_AES(level, mode).expandKey(key);
}

QByteArray HQ_Base_Encrypt_AES::RemovePadding(const QByteArray &rawText, HQ_Base_Encrypt_AES::Padding padding)
{
    if (rawText.isEmpty())
        return rawText;
...
HQ_Base_Encrypt_AES::HQ_Base_Encrypt_AES(Aes level, Mode mode,
                               Padding padding)
...
QByteArray HQ_Base_Encrypt_AES::getPadding(int currSize, int alignment)
{
...
QByteArray HQ_Base_Encrypt_AES::expandKey(const QByteArray &key)
{
...
void HQ_Base_Encrypt_AES::addRoundKey(const quint8 round, const QByteArray &expKey)
{
...
void HQ_Base_Encrypt_AES::subBytes()
{
  QByteArray::iterator it = m_state->begin();
  for(int i = 0; i < 16; i++)
...
void HQ_Base_Encrypt_AES::shiftRows()
{
    QByteArray::iterator it = m_state->begin();
    quint8 temp;
...
//optimized!!
void HQ_Base_Encrypt_AES::mixColumns()
{
  QByteArray::iterator it = m_state->begin();
  quint8 tmp, tm, t;
...
void HQ_Base_Encrypt_AES::invMixColumns()
{
  QByteArray::iterator it = m_state->begin();
  quint8 a,b,c,d;
  for(int i = 0; i < 16; i+=4){
...
// The SubBytes Function Substitutes the values in the
// state matrix with values in an S-box.
void HQ_Base_Encrypt_AES::invSubBytes()
{
    QByteArray::iterator it = m_state->begin();
    for(int i = 0; i < 16; ++i)
...
void HQ_Base_Encrypt_AES::invShiftRows()
{
    QByteArray::iterator it = m_state->begin();
    uint8_t temp;
...
QByteArray HQ_Base_Encrypt_AES::byteXor(const QByteArray &a, const QByteArray &b)
{
...
QByteArray HQ_Base_Encrypt_AES::cipher(const QByteArray &expKey, const QByteArray &in)
{
...
QByteArray HQ_Base_Encrypt_AES::invCipher(const QByteArray &expKey, const QByteArray &in)
{
...
QByteArray HQ_Base_Encrypt_AES::printArray(uchar* arr, int size)
{
    QByteArray print("");
...
QByteArray HQ_Base_Encrypt_AES::encode(const QByteArray &rawText, const QByteArray &key, const QByteArray &iv)
{
...
QByteArray HQ_Base_Encrypt_AES::decode(const QByteArray &rawText, const QByteArray &key, const QByteArray &iv)
{
    if (m_mode >= CBC && (iv.isEmpty() || iv.size() != m_blocklen))
       return QByteArray();
...
QByteArray HQ_Base_Encrypt_AES::removePadding(const QByteArray &rawText)
{
    return RemovePadding(rawText, (Padding) m_padding);
}

QString HQ_Base_Encrypt_AES::f_MadeKey(QString sKey)
{
    QString sKey_Docker = "0123456789ABCDEF";
    int iKeyLength = sKey.length();
    int iDocker_Count = (iKeyLength / 16) + 1;
    int iMod = iKeyLength % 16;
    for (int i = 1; i < iDocker_Count; i++)
    {
        sKey_Docker.append(sKey_Docker);
    }
    return sKey + sKey_Docker.mid(iMod);
}
